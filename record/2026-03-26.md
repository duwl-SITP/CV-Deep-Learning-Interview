...
layout: default
title: 2026-03-26
permalink: /record/2026-03-26.html
questions: 3
source: "[[字节]]"
---
## 1. Flash Attention原理
...
前向传播伪代码的 line 13，更新 $m_{i}^{new}$ 和 $l_{i}^{new}$ 。$l_{i}^{new}$ 是当前 `query` 更新后的 softmax 分母部分， $\widetilde{l}_{ij}$ 是当前块的 softmax 分母部分。由于 softmax 分母部分是 $\sum{e^{x-max(x)}}$ , 我们想要更新这一部分只需要把旧的 $max(x)$ 更新为新的。对于 $l_{i}^{new}$ 和 $\widetilde{l}_{ij}$ :

$$
l_{i}^{new} = e^{m_{i}-m_{i}^{new}}l_{i} + e^{\widetilde{m}_{i}-m_{ij}^{new}}\widetilde{l}_{ij}
$$

可以这样理解这个式子：更新后的 $m_{i}^{new}$ 是当前已处理的块的最大值，对于 $l_{i}^{new}$ 和 $\widetilde{l}_{ij}$ 我们只需要将以旧最大值为基准计算的指数和，重新缩放到以新最大值为基准，就完成了对 softmax 分母部分的修正。

对于 line 16，$O_{i}$ 每次更新时都会乘 $diag(l^{new}_{i})^{-1}$ ，所以相较于 $\widetilde{P}_{ij}V_{j}$ 需要消除旧的分母部分 $diag(l_{i})$ 的影响。此外，乘 $e^{m_{i}-m^{new}_{i}}$ 是为了对分子部分进行修正。
...
因此，Swin Transformer 建立了一个极小的可学习参数表 $\hat{B}$ ：
